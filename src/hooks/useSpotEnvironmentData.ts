...
import type { DailyTideData, WeatherData } from "@/types/fishing";
...
export function useTideData(
  spotId: string | null,
  date: string,
): AsyncState<DailyTideData> {
  const [data, setData] = useState<DailyTideData | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/tide?spotId=${encodeURIComponent(spotId)}&date=${encodeURIComponent(date)}`,
      ...
      .then(async (response) => {
        ...
        return (await response.json()) as DailyTideData;
      })
      .then((payload) => {
        setData(payload);
        setLoading(false);
